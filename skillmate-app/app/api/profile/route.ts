...
export async function POST(req: Request) {
  ...
  const { name, bio, skillsW, skillsH, selectedSkillsH } = await req.json();
  ...
  await pool.execute('DELETE FROM user_has_skill WHERE user_id = ?', [session.id]);
if (Array.isArray(selectedSkillsH) && selectedSkillsH.length) {
  const values = selectedSkillsH.map((sid: number) => `(${session.id}, ${Number(sid)})`).join(',');
  await pool.query(`INSERT INTO user_has_skill (user_id, skill_id) VALUES ${values}`);
}
